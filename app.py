...
<div style="font-size: 0.85rem; color: var(--pa-dove-gray); margin-top: 4px;">{food_kr}</div>
    ...
    if not pd.isna(pairing_notes) and str(pairing_notes).strip():
        pairing_note_text = str(pairing_notes).strip()

        base_rationale = (
            f"<b>{wine_name}</b>, with {flavor_str}, "
            f"creates a complementary match with <b>{food_en}</b>. "
            f"<br><br>"
            f"<span style='color:{BRAND_COLORS['dove_gray']};'>"
            f"Wine pairing note: {pairing_note_text}"
            f"</span>"
        )
    ...
    if pairing_score is not None:
        score_summary = (
            f'<div style="margin-bottom:1.2rem; padding-bottom:1.1rem; '
            ...
            f'color:{BRAND_COLORS["dove_gray"]};"> / 100</span>'
            f'</div>'
            f'<div style="font-size:0.85rem; line-height:1.8;">'
            f'<b>Body &amp; Richness</b>: {body_match:.0f}/100<br>'
            f'<b>Acidity Balance</b>: {acidity_balance:.0f}/100<br>'
            f'<b>Sweetness &amp; Spice</b>: {sweetness_spice:.0f}/100<br>'
            f'<b>Tannin Compatibility</b>: {tannin_compatibility:.0f}/100'
            f'</div>'
            f'</div>'
        )
    else:
    ...
    rationale_text = (
        score_summary
        + f'<div><b>Why this pairing works</b><br>'
        + base_rationale
        + '</div>'
    )
    ...
    result_html = (
        f'<div style="background-color:{BRAND_COLORS["light_gray"]}; '
        f'color:{BRAND_COLORS["mineshaft"]}; '
        f'border:1px solid {BRAND_COLORS["sable_beige"]}; '
